...
describe("collectCodexTrajectories", () => {
	it("builds source-native trajectories for a turn with reasoning, tool calls, and final assistant text", async () => {
		...
		fs.writeFileSync(
			...
			[
				JSON.stringify({
					...
					payload: { id: "s1", cwd: "/tmp/project", model: "codex-test" },
				}),
				JSON.stringify({
					type: "turn_context",
					timestamp: "2026-01-01T00:00:00Z",
					payload: { model: "codex-turn", effort: "high" },
				}),
				JSON.stringify({
					type: "event_msg",
		...
		expect(trajectories[0].steps.map((step) => step.source)).toEqual([
			"user",
			"agent",
			"agent",
		]);
		expect(trajectories[0].steps[1]?.reasoning_content).toBe("reasoning");
		expect(trajectories[0].steps[1]?.tool_calls?.[0]?.function_name).toBe(
			"Read",
		);
		expect(trajectories[0].steps[1]?.observation?.results[0]?.content).toBe(
			"file contents",
		);
	...
	it("keeps exec_command_end as a tool-only projection when no visible assistant text exists", async () => {
		...
		fs.writeFileSync(
			...
			[
				JSON.stringify({
					type: "session_meta",
					timestamp: "2026-01-01T00:00:00Z",
					payload: { id: "s2", cwd: "/tmp/project", model: "codex-test" },
				}),
				JSON.stringify({
					type: "event_msg",
					timestamp: "2026-01-01T00:00:01Z",
		...
		expect(
			trajectories[0].steps[1]?.observation?.results[0]?.content,
		...
		if (records[0].messages[1]?.role !== "tool")
	...
	it("demotes late developer messages instead of emitting invalid system ordering", async () => {
		...
		const rows = [
			...
			},
			{
				...
				payload: {
					type: "message",
					role: "developer",
					content: [{ type: "input_text", text: "extra constraint" }],
				},
				timestamp: "2026-01-01T00:00:02Z",
			},
			{
		...
		const records = await collectCodexRecords(dir);
		expect(records).toHaveLength(1);
		expect(records[0].meta.lossy_reasons).toContain(
			"late_developer_message_demoted",
		);
