...
describe('collectPiTrajectories', () => {
  it('strips think wrappers from Pi reasoning blocks', async () => {
    ...
    expect(trajectories[0].steps[1]?.reasoning_content).not.toContain('<think>');
    expect(trajectories[0].steps[1]?.reasoning_content).not.toContain('</think>');
  });
  ...
  it('attaches tool results and bash executions to the current assistant step observation', async () => {
    ...
    fs.writeFileSync(
      file,
      [
        JSON.stringify({ type: 'session', version: 3, id: 's2', timestamp: '2026-01-01T00:00:00Z', cwd: '/tmp/project' }),
        JSON.stringify({ type: 'model_change', id: 'm1', parentId: null, timestamp: '2026-01-01T00:00:01Z', provider: 'test', modelId: 'model' }),
        JSON.stringify({
          type: 'message',
          id: 'u1',
        ...
        JSON.stringify({
          ...
          message: {
            ...
            content: [
              { type: 'text', text: 'answer' },
              { type: 'toolCall', id: 'call_read', name: 'Read', arguments: { file_path: 'README.md' } },
            ],
          },
        }),
        JSON.stringify({
    ...
    expect(agentStep?.tool_calls?.map((toolCall) => toolCall.function_name)).toEqual(['Read', 'bash']);
    expect(agentStep?.observation?.results).toHaveLength(2);
    expect(agentStep?.observation?.results[0]?.content).toBe('file contents');
    expect(agentStep?.observation?.results[1]?.content).toContain('"command":"ls"');

    const records = await collectPiRecords(dir);
    expect(records).toHaveLength(1);
    expect(records[0].messages.map((message) => message.role)).toEqual(['user', 'assistant', 'tool', 'tool']);
