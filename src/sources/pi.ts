...
export async function collectPiTrajectories(root: string): Promise<AtifTrajectory[]> {
  ...
  for (const file of files.sort()) {
    ...
    const rows = rawRows.map((row, index) =>
      index === 0 ? PiSessionHeaderSchema.parse(row) : PiSessionEntrySchema.parse(row),
    );
    ...
    for (const leaf of leaves) {
      ...
      const trajectory = buildPiTrajectory(pathEntries, header, file, leaves.length > 1);
      if (trajectory) {
...
function buildPiTrajectory(
    ...
    if (entry.type === 'message') {
      ...
      if (role === 'user') {
        ...
        steps.push({
          ...
          extra: buildPiExtra({ qwen_content: content, step_kind: 'user_message' }),
        });
        continue;
      }
  ...
  if (!steps.some((step) => step.source === 'user') || steps.length === 0) return null;
...
function attachPiToolResult(
  ...
  observation.results.push({
    source_call_id: ensurePiToolCallId(hostStep, toolName, asString(msg.toolCallId)),
...
function ensurePiToolCallId(
  ...
  if (toolCallId) {
    const existingById = toolCalls.find((toolCall) => toolCall.tool_call_id === toolCallId);
    if (existingById) {
  ...
  const existingByName = toolCalls.find((toolCall) => toolCall.function_name === toolName);
  if (existingByName) {
    step.tool_calls = toolCalls;
    return existingByName.tool_call_id;
  }

  const nextId = toolCallId ?? `pi_tool_call_${toolName}_${toolCalls.length + 1}`;
  toolCalls.push({
    tool_call_id: nextId,
  ...
  if (Array.isArray(content)) {
    for (const raw of content) {
      ...
      if (type === 'thinking') {
        ...
        if (!rawThinking && asString(block.thinkingSignature)) {
      ...
      if (type === 'toolCall') {
        ...
        tools.set(name, { name });
        toolCalls.push({
          tool_call_id: asString(block.id) ?? `pi_tool_call_${name}_${toolCalls.length + 1}`,
          function_name: name,
...
function projectPiTrajectoryToQwen35Record(trajectory: AtifTrajectory): Qwen35Record {
...
function buildProjectedPiMessages(trajectory: AtifTrajectory): Qwen35Record['messages'] {
...
function collectTrajectoryTools(trajectory: AtifTrajectory): PiToolDefinition[] {
  const tools = new Map<string, PiToolDefinition>();
  for (const step of trajectory.steps) {
...
function formatBash(msg: Record<string, unknown>, lossyReasons: Set<string>): string {
  ...
  const fullOutputPath =
    ...
    asString((msg.details as Record<string, unknown> | undefined)?.fullOutputPath);
...
function buildMeta(messages: Qwen35Record['messages'], seed: Record<string, unknown>) {
...
function readPiProjectionMetadata(value: unknown): PiProjectionMetadata | undefined {
  ...
  if (!endpoint || !ts || !source || !sourceFile || !sourceSessionId) return undefined;
  return {
...
function readPiStepQwenContent(step: AtifStep): Qwen35Content {
  const projection = isRecord(step.extra?.pi_export) ? step.extra.pi_export : undefined;
...
function readPiObservationQwenContent(result: AtifObservationResult): Qwen35Content {
  const projection = isRecord(result.extra?.pi_export) ? result.extra.pi_export : undefined;
  const content = projection?.qwen_content;
  if (typeof content === 'string') return content;
  if (Array.isArray(content)) return content as Qwen35Content;
