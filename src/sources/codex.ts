...
class TurnTrajectoryBuilder {
  ...
  ingest(entry: CodexEntry) {
    ...
    if (entry.type === 'response_item') this.ingestResponseItem(payload, entry.timestamp);
  ...
  ingestResponseItem(payload: Record<string, unknown>, timestamp?: string) {
    ...
    if (type === 'function_call_output') this.ingestFunctionCallOutput(payload, timestamp);
    if (type === 'custom_tool_call') this.ingestCustomToolCall(payload, timestamp);
    if (type === 'custom_tool_call_output') this.ingestCustomToolCallOutput(payload, timestamp);
  }

  ingestEvent(payload: Record<string, unknown>, timestamp?: string) {
    const type = asString(payload.type);
    if (type === 'exec_command_end') this.ingestExecCommandEnd(payload, timestamp);
  ...
  ingestMessage(payload: Record<string, unknown>, timestamp?: string) {
    ...
    if (role === 'assistant') {
      ...
      const nextContent = appendTextContent(readCodexStepQwenContent(step), text);
    ...
    if (role === 'user') {
      const contentValue = text;
      this.steps.push({
        step_id: this.steps.length + 1,
        timestamp,
        source: 'user',
        message: contentValue,
        extra: buildCodexExtra({ qwen_content: contentValue, step_kind: 'user_message' }),
      });
      return;
    }
    ...
    if (role === 'developer' && text) {
      this.steps.push({
        ...
        source: 'system',
        message: text,
        extra: buildCodexExtra({ qwen_content: text, step_kind: 'developer_message' }),
      });
    }
  }
  ...
  ingestReasoning(payload: Record<string, unknown>, timestamp?: string) {
    ...
    if (visible.length > 0) {
      step.reasoning_content = appendParagraphText(step.reasoning_content, visible.join('\n\n'));
      return;
    }
    if (payload.encrypted_content) this.lossyReasons.add('encrypted_reasoning_without_summary');
  }
  ...
  ingestFunctionCall(payload: Record<string, unknown>, timestamp?: string) {
    ...
    const callId = asString(payload.call_id) ?? `codex_tool_call_${this.turnId}_${(step.tool_calls?.length ?? 0) + 1}`;
  ...
  ingestCustomToolCall(payload: Record<string, unknown>, timestamp?: string) {
    ...
    toolCalls.push({
      tool_call_id: callId,
      function_name: name,
      arguments: compactJsonObject({ input: payload.input, status: payload.status }) ?? {},
      extra: buildCodexExtra({
  ...
  ingestFunctionCallOutput(payload: Record<string, unknown>, timestamp?: string) {
  ...
  ingestCustomToolCallOutput(payload: Record<string, unknown>, timestamp?: string) {
  ...
  ingestExecCommandEnd(payload: Record<string, unknown>, timestamp?: string) {
    ...
    this.attachObservationResult(openStep, asString(payload.call_id), 'exec_command', output, output, 'exec_command_end');
  }
  ...
  finalize(): AtifTrajectory | null {
    ...
    if (this.lastAgentMessage && !this.draftAgentStep) {
      ...
      const content = appendTextContent(readCodexStepQwenContent(step), this.lastAgentMessage);
  ...
  ensureDraftAgentStep(timestamp?: string): AtifStep {
    ...
    this.draftAgentStep = {
      ...
      message: '',
      model_name: asString(this.sessionMeta.model),
      extra: buildCodexExtra({ qwen_content: '', step_kind: 'assistant_message' }),
    };
  ...
  prepareObservationHostStep(timestamp?: string): AtifStep {
    ...
    this.openObservationStep = {
      ...
      model_name: asString(this.sessionMeta.model),
      tool_calls: [],
      extra: buildCodexExtra({ qwen_content: '', step_kind: 'synthetic_tool_host' }),
    };
...
export async function collectCodexTrajectories(root: string): Promise<AtifTrajectory[]> {
...
function projectCodexTrajectoryToQwen35Record(trajectory: AtifTrajectory): Qwen35Record {
...
function buildProjectedCodexMessages(trajectory: AtifTrajectory): Qwen35Record['messages'] {
  const messages: Qwen35Record['messages'] = [];

  for (const step of trajectory.steps) {
    if (step.source === 'system') {
      messages.push({ role: 'system', content: readCodexStepQwenContent(step) });
      continue;
    }
    ...
    if (!shouldSkipSyntheticToolHost(step, stepContent)) {
      ...
      if (step.reasoning_content) assistant.reasoning_content = step.reasoning_content;
    ...
    const toolNameById = new Map(
      (step.tool_calls ?? []).map((toolCall) => [toolCall.tool_call_id, toolCall.function_name]),
    );
...
function collectTrajectoryTools(trajectory: AtifTrajectory): CodexToolDefinition[] {
...
function ensureCodexToolCall(
  ...
  const toolCalls = step.tool_calls ?? [];
  if (callId) {
    const existingById = toolCalls.find((toolCall) => toolCall.tool_call_id === callId);
    if (existingById) {
      step.tool_calls = toolCalls;
  ...
  const existingByName = toolCalls.find((toolCall) => toolCall.function_name === functionName);
  if (existingByName) {
    step.tool_calls = toolCalls;
...
function buildCodexMeta(messages: Qwen35Record['messages'], seed: Record<string, unknown>) {
  const assistantMessages = messages.filter((message) => message.role === 'assistant');
  const toolMessages = messages.filter((message) => message.role === 'tool');
  const lossyReasons = Array.isArray(seed.lossy_reasons)
    ? seed.lossy_reasons.filter((reason): reason is string => typeof reason === 'string')
    : [];
...
function readCodexProjectionMetadata(value: unknown): CodexProjectionMetadata | undefined {
  ...
  return {
    ...
    lossy_reasons: Array.isArray(raw.lossy_reasons)
      ? raw.lossy_reasons.filter((reason): reason is string => typeof reason === 'string')
      : [],
  };
}

function readCodexStepQwenContent(step: AtifStep): Qwen35Content {
  const projection = isRecord(step.extra?.codex_export) ? step.extra.codex_export : undefined;
...
function readCodexObservationQwenContent(result: AtifObservationResult): Qwen35Content {
  const projection = isRecord(result.extra?.codex_export) ? result.extra.codex_export : undefined;
  const content = projection?.qwen_content;
  if (typeof content === 'string') return content;
  if (Array.isArray(content)) return content as Qwen35Content;
...
function shouldSkipSyntheticToolHost(step: AtifStep, content: Qwen35Content): boolean {
  const projection = isRecord(step.extra?.codex_export) ? step.extra.codex_export : undefined;
  return (
...
function appendTextContent(content: Qwen35Content, text: string): Qwen35Content {
  ...
  if (lastBlock?.type === 'text') {
    lastBlock.text = lastBlock.text.length > 0 ? `${lastBlock.text}\n\n${text}` : text;
    return blocks;
  }
...
function appendParagraphText(existing: string | undefined, text: string): string {
...
function compactJsonObject(value: unknown): Record<string, unknown> | undefined {
  if (!isRecord(value)) return undefined;
  const compacted = compactJsonValue(value);
  return isRecord(compacted) && Object.keys(compacted).length > 0 ? compacted : undefined;
}
...
function compactJsonValue(value: unknown): unknown {
  ...
  if (Array.isArray(value)) {
    return value.map((entry) => compactJsonValue(entry)).filter((entry) => entry !== undefined);
  }
  if (isRecord(value)) {
